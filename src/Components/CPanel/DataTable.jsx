...
const DataTable = ({ title, data, fetchData, total, onDelete, rows, actions, userFilter = false }) => {
    ...
    const handleEditPage = (id) => {
        navigate(`/cpanel/${title.charAt(0).toLowerCase() + title.slice(1)}/edit/${id}`);

    }
    // When you want to navigate with data:
    const handleAddPage = () => {
        navigate(`/cpanel/${title.charAt(0).toLowerCase() + title.slice(1)}/add`);
    };
    ...
    return <>
        <div className="mb-4 flex flex-col sm:flex-row sm:items-center sm:justify-between">
            <Link
                to="/cpanel"
                className="flex items-center justify-center px-4 py-2 mb-2 sm:mb-0 bg-blue-500 hover:bg-blue-600 text-white rounded-lg shadow-md transition-all duration-300 ease-in-out transform hover:scale-105 hover:shadow-lg"
            >
            ...
            {actions?.includes('add') && (
                <Button
                    ...
                    color="success"
                    startIcon={<MdAdd />}
                    className="bg-green-500 hover:bg-green-600 text-white rounded-lg shadow-md transition-all duration-300 ease-in-out transform hover:scale-105 hover:shadow-lg"
        ...
        <Paper className="dark:bg-gray-800 p-4 rounded-lg shadow-md">
            ...
            {data === 'loading' ? <LoadingScreen /> :
                data === 'error' ? <ErrorText handleRefresh={fetchData} /> :
                    !data?.length ? <ErrorText handleRefresh={fetchData} text={`No ${title} found`} /> :
                                                ...
                                                {actions?.includes('edit') && <TableCell>
                                                    {row.role === allRoles.SA ? '' :
                                                        <Tooltip title="edit" arrow placement="top">
                                                            <IconButton onClick={() => handleEditPage(row._id)} color="primary">
                                                                <MdEdit size={25} />
                                                            </IconButton>
                                                        </Tooltip>}
                                                </TableCell>}
